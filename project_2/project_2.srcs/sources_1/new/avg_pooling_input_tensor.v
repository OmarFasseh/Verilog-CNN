...
parameter filter =6; //24 clk // the number of photos
...
/*
Firstly 
check for start I send the first photo for average layer module that calculated the average for one photo 
and make start = 0 to no enter it again and wait to 8 ticks to finish it and then take output 
and make reset to inner module to can send anther photo to it and then make also for rest of photos
...
if(start==0) //send first photo when start = 0 
begin
...
if(i<filter && (j==10)) // check on clk ticks and the index of 3d array bigger than number of photos to pass new input  
begin
      reset=0;
       j=0;    
...
if(j==8)//this time of take photo of inner module after 8 clock tickes
begin
