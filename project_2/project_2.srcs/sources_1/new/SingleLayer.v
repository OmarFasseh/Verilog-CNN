`timescale 1ns / 1ps
module SingleLayer(input_fc, clk, start_fc, reset,output_fc,test_multi,test_weights,test_output);
...
weights_Memory #(.DATA_WIDTH(DATA_WIDTH),.parallel_fc_PE(OUTPUT_SIZE),.file(file),.fc_columns(INPUT_SIZE),.tot_weight_size(INPUT_SIZE*OUTPUT_SIZE))
wm(.clk(clk),
